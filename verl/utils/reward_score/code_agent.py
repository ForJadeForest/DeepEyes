...
def compute_score(predict_str: str, ground_truth: str, extra_info=None) -> dict:
    ...
    if count_vision_1 != count_vision_2:
        print("[REWARD DEBUG INFO]: Vision tags count mismatch")
        is_format_error = True

    tool_call_1 = predict_str.count("<tool_call>")
    tool_call_2 = predict_str.count("</tool_call>")
    if tool_call_1 != tool_call_2:
        print(f"[REWARD DEBUG INFO]: Tool call count mismatch, {tool_call_1} != {tool_call_2}")
        print(f"[REWARD DEBUG INFO]: {predict_str}")
        is_format_error = True

    if (
        tool_call_1 > 0
        and tool_call_1 == tool_call_2
        and not ("[CODE RUN ERROR]" in predict_str and "[TOOLERROR]" in predict_str)
    ):
    ...
    if count_answer_1 != 1 or count_answer_2 != 1:
        print(f"[REWARD DEBUG INFO]: Answer tags count mismatch, {count_answer_1} != {count_answer_2}")
        print(f"[REWARD DEBUG INFO]: {predict_str}")
        is_format_error = True

    if not predict_str.endswith("</answer>"):
        print("[REWARD DEBUG INFO]: Answer tag not at the end")
        is_format_error = True
    ...
    tool_reward = 0.5 if give_tool_reward and acc_reward > 0.5 else 0.0
    format_reward = 0 if is_format_error else 1.0
    final_score = 1.0 * acc_reward + 0.5 * format_reward + 1.0 * tool_reward
